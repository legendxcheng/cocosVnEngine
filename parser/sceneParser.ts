...
import { uniqueBy, compareAssets } from '../../utils';
...
export const sceneParser = (
  ...
  // 过滤掉 commandType.comment 的语句
  const filteredSentenceList = sentenceList.filter(sentence => sentence.command !== commandType.comment 
    && sentence.content.indexOf('//') == -1);

  // 开始资源的预加载
  assetsList = uniqueBy(assetsList, compareAssets); // 去重
  assetsPrefetcher(assetsList);

  return {
    sceneName: sceneName, // 场景名称
    sceneUrl: sceneUrl,
    sentenceList: filteredSentenceList, // 语句列表
